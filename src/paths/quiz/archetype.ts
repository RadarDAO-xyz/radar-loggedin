...
function determineArchetype(percentages: AnswerFrequencies) {
    for (let i = 0; i < 5; i++) {
        ...
        if (percentage >= 80) return id;
        if (between(40, percentage, 50) && between(40, percentageNext(1), 70)) return id + 10;
        if (between(40, percentage, 50) && between(40, percentageNext(2), 70)) return id + 14;
        if (between(40, percentage, 50) && between(40, percentageNext(3), 70)) return id + 17;
        if (between(40, percentage, 50) && between(40, percentageNext(4), 70)) return id + 19;
        if (between(40, percentage, 70)) return id + 5;
    }
    return 21;
}
